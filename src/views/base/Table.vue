<template>
  ...
  <div>
    ...
    <CCard>
      <CCardHeader>
        <slot name="header">
          <CRow class="d-flex justify-content-between">
            ...
            <CButton :to="{name: titleButton.link}"
                     class="mr-3"
                     color="primary"
                     shape="pill"
      ...
      <CCardBody>
        <CDataTable items-per-page-select table-filter
                    :hover="hover"
                    :striped="striped"
                    :bordered="bordered"
                    :small="small"
          ...
          {{edit.route}}
          <CDropdownItem v-show="statusUpdate" @click="openStatusModal(item)">update status</CDropdownItem>
          <CDropdownItem @click="openModal(item)"
                         class="text-danger"
                         v-show="deleteExist">delete</CDropdownItem>
    ...
    <CModal :show.sync="statusUpdateModal"
            :no-close-on-backdrop="true"
            :centered="true"
            title="Warning!"
            size="sm"
      ...
      <CListGroup v-for="(item,index) in statusOptions" :key="index">
        <CListGroupItem @click="status = item.value" class="list-gp my-2">
